...
import TextFieldInput from "./inputbox";
...
const EditPatientModal = ({showModal,setShowModal,data,fetchData}) => {   
  ...
  const formik = useFormik({
    initialValues: {
        ...
        VisitorTypeID:  visitorTypes?.find(obj => obj.id === data?.VisitorTypeID?._id)?.type || '',
        patientTypeID:  patientTypes?.find(obj => obj.id === data?.patientTypeID?._id)?.type || '', 
  ...
  const renderFilePreview = (fileUrl) => {
    ...
    if (['jpg', 'jpeg', 'png', 'gif'].includes(fileExtension)) {
      return <img src={fileUrl} alt="file preview" style={{ maxWidth: '100%',width: '100%',height: '5rem' }} />;
    } else if (fileExtension === 'pdf') {
      return <iframe src={fileUrl} title="PDF preview" style={{ width: '100%', height: '5rem' }} />;
    } else {
                      ...
                      {formFields.map((field, i) => (
                        <div
                          ...
                          {field.type === "dropdown" ? (
                            ...
                            </>) :field.type === "file" ? ( formik.values.fileUrl ? <div className="relative"><span className="absolute -top-3 -right-0 hover:scale-150 transition-all duration-300" onClick={()=>{
                               formik.setFieldValue("fileUrl", null);  
                            }}><CloseIcon/> </span>{renderFilePreview(formik.values.fileUrl)}</div> :
                            formik.values.patientTypeID === "ECONOMY" && <div className="w-full max-w-[15rem]">
                            <TextFieldInput
                              id="file"
                              name="file"
                              type="file"
